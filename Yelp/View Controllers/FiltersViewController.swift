...
@objc protocol FiltersViewControllerDelegate {
    @objc optional func filtersViewController(filtersViewController: FiltersViewController, didUpdateFilter filters: [String], didUpdateDeal deal: Bool, didUpdateSort sort: String)
}
...
class FiltersViewController: UIViewController {
    ...
    @IBAction func onSearch(_ sender: UIBarButtonItem) {
        ...
        if switchSortOn == true {
            sortValue="highestRated"
        }

        if filters.count > 0 || switchSortOn == true || dealSwitch == true {
            delegate?.filtersViewController!(filtersViewController: self, didUpdateFilter: filters, didUpdateDeal: dealSwitch, didUpdateSort: sortValue)
        }
        dismiss(animated: true, completion: nil)
    }
}
...
extension FiltersViewController: SortCellDelegate {
    func sortCellDidSwitchChanged(sortCell: SortCell,didChangeValue value: Bool) {
        switchSortOn = value
    }
}
